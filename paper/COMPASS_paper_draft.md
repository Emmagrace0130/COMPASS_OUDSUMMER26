...
### 4.7 Isolating generation-time noise

Section 4.6's clinical-query v2 result leaves an open question: once retrieval is made byte-identical across demographic variants, why do composite-score gaps (0.26–0.37 mean absolute, Table 8) persist at all? Two explanations are possible — a genuine effect of demographic wording on generation, or ordinary run-to-run stochasticity in the LLM and judge. The equity pilot's 2 repeats per variant cannot distinguish these; we ran a dedicated experiment to do so.

Each of the five scenarios' base question — unmodified, no demographic variation — was submitted 8 times to the default pipeline (40 runs total) and scored with the same judge rubric. Because the question string never changes, retrieval is trivially identical on every repeat, so any score spread here is pure generation-and-judge noise, with a sample size four times larger than any single variant in the equity pilot.

**Table 9. Noise floor: the same unmodified question, no demographic variation, run 8 times per scenario (n=40).**

| Scenario | n | Mean | SD | Min | Max |
|---|---:|---:|---:|---:|---:|
| S1 | 8 | 3.58 | 0.43 | 3.33 | 4.33 |
| S2 | 8 | 4.46 | 0.23 | 4.33 | 5.00 |
| S3 | 8 | 4.25 | 0.43 | 3.33 | 5.00 |
| S4 | 8 | 3.83 | 0.50 | 3.33 | 4.67 |
| S5 | 8 | 4.54 | 0.23 | 4.00 | 4.67 |
| **Pooled mean \|deviation from own scenario mean\|** | | | | | **0.30** |

The pooled noise floor, 0.30, sits within a narrow band around every demographic-variant gap from the v2 equity pilot: race 0.27 (0.90× the floor), gender 0.26 (0.87×), insurance 0.28 (0.93×), county 0.34 (1.13×), criminal-justice involvement 0.37 (1.23×), and the two-factor variant 0.26 (0.87×). None exceeds the floor by a margin that a sample of 2 repeats per variant (against 8 for the floor) could distinguish from chance. **We find no evidence, once retrieval bias is corrected, that COMPASS's answer quality depends on the demographic dimensions tested** — the score gaps that looked like a residual equity concern are consistent with being ordinary generation noise, full stop.

This reframes Section 4.6's equity finding as a two-part result rather than a partial one: the demographic-equity pilot found a real, mechanistic bias (retrieval sensitivity to irrelevant patient wording), fixed by a deterministic mitigation with no cost; and once that mitigation is applied, the remaining score variation is not distinguishable from the variation a completely neutral, demographic-free repeat of the same question already produces. Both halves are limited by the same small scenario count (five) discussed in Section 5.1, and a noise floor built from more repeats or more scenarios would sharpen the comparison further, but the current result gives no reason to suspect a hidden effect at the resolution this pilot can measure.
...
- **Corpus duplication and an unpopulated topic.** About 41 of the 1,046 PDF files are redundant copies (roughly 1,005 distinct; Section 3.1), including the 2022 CDC opioid-prescribing guideline filed three times. Those 41 files have since been quarantined out of the ingestion path, but the index has not been rebuilt without them, so the results in this paper still reflect the un-deduplicated corpus and a rebuild-and-re-evaluate pass remains open work. Duplicates can fill several retrieval slots with the same passage and defeat per-file diversity limits. T10 (AI in Addiction Medicine) has no ingested documents.
...
Zitu, M. M., Owen, D., Manne, A., Zhu, Y., Binkheder, S., & Li, L. (2026). Artificial intelligence for opioid safety surveillance from clinical text: a clinically focused review. *Journal of Clinical Medicine, 15*(4), 1649. https://doi.org/10.3390/jcm15041649
...
- Generation-time noise floor: `evaluation/run_noise_floor.py`; results `evaluation/results/noisefloor_20260923_185849.json`
