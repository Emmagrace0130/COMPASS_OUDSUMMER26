...
Repeating the pilot with clinical-query retrieval (Table 8) gave a partial, suggestive improvement. Race and gender gaps shrank (mean absolute gap 0.42 to 0.12 and 0.39 to 0.17), and recommended-medication overlap with the base rose across dimensions (race 0.57 to 0.82, gender 0.72 to 0.85). Answer quality on the standard 20 questions was unchanged: two runs scored composites of 0.823 and 0.840, inside the plain baseline's range of 0.803–0.850, at about 0.8 s more latency per question.

**Table 8. Equity pilot before and after clinical-query retrieval: mean absolute composite gap, medication overlap, and cited-source overlap versus the base scenario.**

| Dimension varied | Mean \|gap\| orig. | clin.-query | Med. overlap orig. | clin.-query | Source overlap orig. | clin.-query |
|---|---:|---:|---:|---:|---:|---:|
| Race/ethnicity | 0.42 | 0.12 | 0.57 | 0.82 | 0.77 | 0.79 |
| Gender | 0.39 | 0.17 | 0.72 | 0.85 | 0.72 | 0.94 |
| Insurance | 0.24 | 0.37 | 0.80 | 0.85 | 0.59 | 0.49 |
| County | 0.56 | 0.22 | 0.67 | 0.87 | 0.87 | 0.78 |
| Criminal-justice involvement | 0.39 | 0.42 | 0.80 | 0.90 | 0.85 | 0.72 |
| Two factors | 0.48 | 0.39 | 0.75 | 0.72 | 0.60 | 0.48 |

It did not remove the retrieval sensitivity. Source overlap for race was unchanged (0.77 to 0.79) and fell for insurance, criminal-justice, and two-factor variants, because the rewriter is itself an LLM that paraphrases differently-worded variants differently: it removes one source of variation and introduces another. The base scenario's own composite spread also doubled (mean deviation 0.12 to 0.27) and base scenario scores changed, so the gap reductions are not cleanly comparable with the original pilot, and with five scenarios per dimension they may partly be chance. We read this as evidence that the approach is worth pursuing — most plausibly with a deterministic, rule-based removal of descriptors rather than an LLM rewrite — and not as a demonstrated fix.
...
- Clinical-query mitigation: `compass-app/backend/rag.py` (`run_ollama_clinical_query`, `REWRITE_PROMPT`), opt-in `clinical_query` field on `/chat`, `--clinical-query` flag in `run_equity.py` and `run_eval.py`; results `evaluation/results/equity_20260921_194614_clinicalquery.json`, `eval_20260921_194907_clinicalquery.csv`, `eval_20260921_195200_clinicalquery.csv`
