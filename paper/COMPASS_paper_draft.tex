...
Repeating the pilot with clinical-query retrieval (Table~\ref{tab:equitycq}) gave a partial, suggestive improvement. Race and gender gaps shrank (mean absolute gap 0.42 to 0.12 and 0.39 to 0.17), and recommended-medication overlap with the base rose across dimensions (race 0.57 to 0.82, gender 0.72 to 0.85). Answer quality on the standard 20 questions was unchanged: two runs scored composites of 0.823 and 0.840, inside the plain baseline's range of 0.803--0.850, at about 0.8\,s more latency per question.

\begin{table}[h]
\centering
\caption{Equity pilot before and after clinical-query retrieval. Mean absolute composite gap, medication overlap, and cited-source overlap versus the base scenario.}
\label{tab:equitycq}
\begin{tabular}{lrrrrrr}
\toprule
 & \multicolumn{2}{c}{Mean $|$gap$|$} & \multicolumn{2}{c}{Med.\ overlap} & \multicolumn{2}{c}{Source overlap} \\
Dimension varied & orig. & clin.-query & orig. & clin.-query & orig. & clin.-query \\
\midrule
...
\begin{itemize}[leftmargin=*]
  ...
  \item Clinical-query mitigation: \path{compass-app/backend/rag.py} (\path{run_ollama_clinical_query}, \path{REWRITE_PROMPT}), opt-in \path{clinical_query} field on \texttt{/chat}, \path{--clinical-query} flag in \path{run_equity.py} and \path{run_eval.py}; results \path{evaluation/results/equity_20260921_194614_clinicalquery.json}, \path{eval_20260921_194907_clinicalquery.csv}, \path{eval_20260921_195200_clinicalquery.csv}
