...
\date{\small Draft --- September 25, 2026. First full-draft pass; see status notes throughout.}
...
\subsection{Isolating generation-time noise}

Section~4.6's clinical-query v2 result leaves an open question: once retrieval is made byte-identical across demographic variants, why do composite-score gaps (0.26--0.37 mean absolute, Table~\ref{tab:equitycq}) persist at all? Two explanations are possible --- a genuine effect of demographic wording on generation, or ordinary run-to-run stochasticity in the LLM and judge. The equity pilot's 2 repeats per variant cannot distinguish these; we ran a dedicated experiment to do so.
...
\begin{table}[h]
\centering
\caption{Noise floor: the same unmodified question, no demographic variation, run 8 times per scenario ($n=40$).}
...
\midrule
Pooled mean $|$deviation from own scenario mean$|$ & \multicolumn{4}{r}{\textbf{0.30}} \\
\bottomrule
\end{tabular}
\end{table}

The pooled noise floor, 0.30, sits within a narrow band around every demographic-variant gap from the v2 equity pilot: race 0.27 (0.90$\times$ the floor), gender 0.26 (0.87$\times$), insurance 0.28 (0.93$\times$), county 0.34 (1.13$\times$), criminal-justice involvement 0.37 (1.23$\times$), and the two-factor variant 0.26 (0.87$\times$). None exceeds the floor by a margin that a sample of 2 repeats per variant (against 8 for the floor) could distinguish from chance. \textbf{We find no evidence, once retrieval bias is corrected, that COMPASS's answer quality depends on the demographic dimensions tested} --- the score gaps that looked like a residual equity concern are consistent with being ordinary generation noise, full stop.
...
\bibitem[Tennessee Department of Health, 2025]{tdh2025} Tennessee Department of Health, Office of Informatics and Analytics. (2025). \emph{2023 Tennessee drug overdose deaths}. \url{https://www.tn.gov/content/dam/tn/health/documents/pdo/death-report/2023_Tennessee_Drug_Overdose_Death_Report.pdf}
...
\bibitem[Zitu et al., 2026]{zitu2026} Zitu, M. M., Owen, D., Manne, A., Zhu, Y., Binkheder, S., \& Li, L. (2026). Artificial intelligence for opioid safety surveillance from clinical text: a clinically focused review. \emph{Journal of Clinical Medicine, 15}(4), 1649. \url{https://doi.org/10.3390/jcm15041649}

\end{thebibliography}

References above combine sources already vetted for this project (from \path{SUD150_Citations_by_Category.docx}) with six technical/data citations added for this draft from live web research (Lewis et al.\ 2020; Gao et al.\ 2023; Edge et al.\ 2024; Nananukul \& Kejriwal 2026; Nwobi et al.\ 2026; Tennessee Department of Health 2025) --- all six, and the Section~1 overdose figure, were verified directly against their primary sources (author lists cross-checked against arXiv/journal listings; the TDH figure quoted verbatim from the agency's own report) before this draft.
...
\begin{itemize}[leftmargin=*]
  ...
  \item Generation-time noise floor: \path{evaluation/run_noise_floor.py}; results \path{evaluation/results/noisefloor_20260923_185849.json}
